...
// Local cache for trending search results (24 hours TTL - different from user selection cache)
// This caches the YouTube search results, not user selections
const trendingSearchCache = new Map();
...
export default {
    ...
    async execute(interaction, client) {
        try {
            ...
            const searchCacheKey = `${region}:${genre}`;
            
            // Check search cache (for YouTube API results)
            const cached = trendingSearchCache.get(searchCacheKey);
            if (cached && (Date.now() - cached.timestamp) < SEARCH_CACHE_TTL) {
                logger.debug('Using cached trending search data', { searchCacheKey });
            ...
            // Cache the search results (24 hours)
            trendingSearchCache.set(searchCacheKey, {
                tracks: uniqueTracks,
                timestamp: Date.now()
            });
...
async function sendTrendingResponse(interaction, client, tracks, region, genre, count) {
    ...
    // Store tracks for user selection using CacheManager
    ...
    if (client.cacheManager) {
        ...
        setTimeout(() => {
            client._trendingCache?.delete(userCacheKey);
        }, 5 * 60 * 1000);
    }
}

/**
 * Cleanup old search cache entries (run periodically)
 */
setInterval(() => {
    const now = Date.now();
    for (const [key, value] of trendingSearchCache.entries()) {
        if (now - value.timestamp > SEARCH_CACHE_TTL) {
            trendingSearchCache.delete(key);
        }
    }
