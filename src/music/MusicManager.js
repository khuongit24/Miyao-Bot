...
export class MusicManager extends EventEmitter {
            ...
            config.lavalink.nodes,
            {
                ...
                nodeResolver: (nodes) => {
                    return this.nodeMonitor.getBestNode(this.shoukaku) || 
                           [...nodes.values()].find(node => node.state === 2);
                }
            }
        );
    ...
    _performGradientCleanup(level, heapUsedMB) {
        ...
        logger.info(`Cleanup completed (${level}): evicted ${evictedCount} cache entries, removed ${cleanedQueues} idle queues in ${duration}ms`);
