...
class Favorites {
    ...
    static remove(userId, trackUrl) {
        try {
            const db = getDatabaseManager();
            const result = db.execute('DELETE FROM favorites WHERE user_id = ? AND track_url = ?', [userId, trackUrl]);
    ...
    static removeById(userId, favoriteId) {
        try {
            const db = getDatabaseManager();
            const result = db.execute('DELETE FROM favorites WHERE id = ? AND user_id = ?', [favoriteId, userId]);
    ...
    static count(userId) {
        try {
            const db = getDatabaseManager();
            const result = db.queryOne('SELECT COUNT(*) as count FROM favorites WHERE user_id = ?', [userId]);
            return result.count;
        } catch (error) {
    ...
    static isFavorited(userId, trackUrl) {
        try {
            const db = getDatabaseManager();
            const result = db.queryOne('SELECT id FROM favorites WHERE user_id = ? AND track_url = ?', [
                userId,
                trackUrl
            ]);
            return !!result;
    ...
    static getRandom(userId, limit = 25) {
        try {
            const db = getDatabaseManager();
            const favorites = db.query('SELECT * FROM favorites WHERE user_id = ? ORDER BY RANDOM() LIMIT ?', [
                userId,
                limit
            ]);
